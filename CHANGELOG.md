...
## 0.2.16 — 适配 DSH 0.1.6-alpha.2：管家按钮失效 + 会话标题回归

> 版号说明：本次修复原按 0.2.15 提交，但 0.2.15 在 npm 首次发布时被登记为
> **staged（待 2FA 批准）**且此后 stage 列表为空，该版号已被占用无法重发，
> 遂改用 0.2.16（同时避开 `wip/knowledge-0.2.15` 分支的号段）。代码内容一致。
...
- **诉讼费测算**：财产案件受理费分段累计（50 元起）、执行申请费、保全申请费（5000 元封顶）、
  支付令（受理费 1/3）、公示催告、撤销仲裁裁决、破产（减半封顶 30 万）、管辖权异议；
  非财产案件（离婚/人格权/其他/知产/劳动争议/行政）；调解撤诉与简易程序**不重复减半**；受理费反算标的额。
- **律师费测算**：按标的额分段累进区间（山东 2017 / 北京 / 市场参考三套标准）、计件、计时、
  风险代理分档上限（18%/15%/12%/9%/6%，司发通〔2021〕87 号）与禁止风险代理案件提示。
- **利息测算**：1 年期 / 5 年期以上 LPR 分段计息（内置 2019-08-20→2026-08-20 完整月度报价表）、
  倍数、固定利率、365/360 口径、算头不算尾/算头算尾。
- **违约金测算**：日万分之 / 年利率 / 月利率 / LPR 倍数四种口径，并对照 4 倍 LPR（民间借贷上限）
  与 1.5 倍 LPR、1.5 倍 LPR×1.3 参考值。
- **迟延履行加倍利息**：未清偿金钱债务 × 日万分之一点七五 × 迟延天数（法释〔2014〕8 号）。
- **期限计算**：次日起算、月/年「到期月对应日」、末日遇法定休假日顺延（内置 2025–2026 节假日与调休表）、
  工作日口径；14 条法定期间预设（上诉期/答辩期/举证/再审/执行/仲裁时效/复议/行政诉讼/国家赔偿等）。
- **日期差计算**：自然日、工作日、周数。
- **金额大写**：人民币大写（壹佰贰拾叁万元整）+ 元角分拆分。

### 二、入口
- 「技能与工具」面板新增第三个选项卡**小工具**（分组卡片 → 详情页表单，即输即得，可复制结果）；
- 输入框「技能」按钮菜单新增「测算工具」入口；`/` 命令新增「打开律师小工具」；
- 新增 agent 工具 **legal_calc**（action + input，纯计算无副作用），会话里直接算。

### 三、界面设计优化（前端设计规范）
- **宽屏双栏**：左侧参数卡、右侧结果卡**粘性跟随**（改一个数立刻看到结果；<1180px 自动改上下堆叠）。
- **分段控件**：测算类型/审理程序/结案方式/LPR 品种/天数口径等少量选项由下拉改为原生 radio 分段控件（键盘与读屏可用）。
- **输入升级**：金额右对齐 + `tabular-nums`，带「元 / % / 倍 / 万分之 / 小时」单位后缀；金额快捷值（1万–1000万）带选中态。
- **结果层级**：主结论独立「英雄区」（22–30px/800 字重），其余指标进自适应网格，明细表带斑马纹与表头。
- **依据折叠**：法条与口径默认收起为「依据与口径 · N 条」，需要时展开（页眉常驻主要依据）。
- **逐字段校验**：失焦即提示（`aria-invalid` + `aria-describedby` + `role="alert"`），整体测算失败时兜底提示空着的必填金额。
- **可访问性**：所有可点元素补 `:focus-visible` 焦点环（2px，3:1 对比）；开关/分段用原生控件 + 视觉隐藏 input；
  复制结果带 `role="status"` 播报与 `execCommand` 回退；`prefers-reduced-motion` 下关闭位移与过渡；
  注释文字对比度从 `--lit-ink-subtle`（约 2.5:1）提到 `--lit-ink-muted`（约 5.3:1）。
- 验收：Playwright 脚本扩到 **25 条断言**（面板三 tab 回归、双栏几何关系、分段控件、单位后缀、失焦校验、
  折叠、复制反馈、深色主题、375px 无横向溢出与上下堆叠），另截图留档 `docs/design/calc-0214-*.png`。

### 三、规则与数据来源（全部落在代码注释与详情页「依据」中）
- 《诉讼费用交纳办法》第十三至十八条；司发通〔2021〕87 号；法释〔2014〕8 号；
  合同编通则解释第六十五条；民间借贷司法解释 4 倍 LPR；
  LPR 月度报价（中国人民银行授权全国银行间同业拆借中心，转录中国银行 LPR 页）；
  国办发明电〔2024〕7 号、〔2025〕7 号（2025/2026 节假日）。
- 校验：`scripts/verify-calculators.mjs`（73 条断言全绿，含手工核算的确定值）。

## 0.2.13 — 数据收敛回「任务/日程」两类 + 日历同步按业务口径重写

### 一、数据模型：keydate 退役，回到两类
- `ItemType` 回到 `event | task | both`（`both` 保留）。0.2.12 引入的 `type='keydate'` 删除：
  关键日期本来就是日程，只是搬家时多开了一个类型（实测 4 种 type 里 both 0 实例、消费端 40 处按 type 分派）。
- 读时归一 `normalizeItemType`（历史 keydate → event）+ 落盘迁移（备份
  `items.json.bak-keydate-merge-0.2.13`、幂等标记 `.agentlex-keydate-merged-0.2.13`）。
- 消费端 23 处 type 分派改为按属性判断：日程 = event/both；期限来源 = 带 ruleId/baseDate。
- `case.keyDates` 退化为**兼容投影**（= 该案全部日程），仅供旧读接口；期限闸门/巡检/UI 口径不变。
  计划 0.2.14 拆除该壳。`addKeyDate`/`upsertKeyDate` 默认写 `source='keydate'` 便于识别。

### 二、日历同步：规则集中到一处，按业务意义筛
- 新增 `src/domains/calendar-sync/rules.ts`——「什么该进日历」的唯一判定处。
- 只同步「今天及以后」的日程；过去的日程是历史时间轴，不进日历（**逾期只属于任务**）。
- 有时间点必带时间点（不再退化成全天）；无时间才是全天。
- 标题格式「【事项标题】案件名」；案件名统一查 registry（此前监听路径退成编号 `【开庭】2026-021`）。
- **不进日历**：答辩期 / 举证期（实践中废止，属噪音）、送达（当天收到无意义）、
  当事人的履行期限（对方的付款义务，与我方无关）。
- **必须进日历**：上诉期、起诉期、申请仲裁时效等影响我方程序权利的实质期限（即使无时间点）。
- 任务严格按 `calendarSyncTasks` 开关；同一「归属+日期+标题」去重，**优先保留带 time 的那条**（修全天化）。
- `enforcement.apply`（申请执行期限届满）**停用**：原 scope 挂在执行程序上，语义错误；
  重新启用前需补「scope 支持多值 + 已有执行案判定」。

### 三、界面
- 案件搜索**默认包含已归档案件**（此前先剔归档再过滤 → 搜不到）；归档卡片加「已归档」标记 + 结果提示。
- 卡片/详情页编号统一为系统字体栈 + `tabular-nums`（原来硬编码 'Microsoft YaHei'，本机未装 →
  静默 fallback 成 Monaco/Menlo，0 带斜杠且与非诉的等宽不一致），字重提到 800、字号 20px。

### 四、数据清理与工具
- 新增 `scripts/cleanup-items-0213.mjs`（干跑默认，`--apply` 落盘，带备份）：
  同名同日重复合并（保留信息最全的一条并补齐缺失字段）、动作类改回任务、`--drop-ids` 删除残留条目、
  同步清理日历台账陈旧键。
- 管家 SOP 增「期限登记纪律」：不登记当事人履行期限；申请执行期限只在尚未申请执行时登记。

### 五、验证
- 新增 `scripts/verify-calendar-sync-0213.mjs`（30 断言全绿，含真实数据干跑）。
- 全量套件 24/25 通过，唯一失败是既有失效脚本 `verify-stage-expansion.mjs`（改前即红，用废弃 stageId trial）。
- `tsc` 与基线一致（99 个既有 vendor 报错，无新增）。
