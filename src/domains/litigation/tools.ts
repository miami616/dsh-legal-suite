...
const PARAMETERS = {
  ...
  groupName: { type: 'string', description: '任务组（阶段）名称，如 一审阶段' },
  taskId: { type: 'string', description: '任务 id（upsert_subtask/delete_subtask/toggle_check/upsert_check 必填）' },
  ...
  subtaskTitle: { type: 'string', description: '子任务标题' },
  checklistId: { type: 'string', description: '检查项 id（toggle_check/upsert_check 需要 caseId+groupId+taskId+checklistId 四件套）' },
  checklistText: { type: 'string', description: '检查项内容（upsert_check 创建检查项；同时传 checklistId 则更新该检查项）' },
