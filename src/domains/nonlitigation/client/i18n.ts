...
export function dictionary(): NonLitigationKey {
  const lang = typeof document !== 'undefined' ? document.documentElement.lang : 'zh'
  return lang.toLowerCase().startsWith('en') ? { ...en } : { ...zh }
}

/** Translate a key with optional {name} template params (current language). */
export function tt(key: keyof NonLitigationKey, values?: TranslateValues): string {
  return t(dictionary(), key, values)
}

/** Interpolate {name} placeholders; unknown key falls back to the key itself. */
export function t(dict: NonLitigationKey, key: keyof NonLitigationKey, values?: TranslateValues): string {
