...
import { useImagePreview } from "./ImagePreviewContext.tsx";
...
import FileSearchResults from "@/components/search/FileSearchResults";
import type { FilePreviewFocusTarget } from "@/types/filePreview";
import {
  ancestorDirectoryPaths,
  firstMatchLine,
} from "@/utils/workspaceSearchNavigation";
import { REVEAL_NODE_WAIT_FRAMES } from '@/components/directory-panel/constants';
import {
  getFolderName,
  waitForTreeFrame,
} from '@/components/directory-panel/pathDisplay';
import { useDirectorySearch } from '@/components/directory-panel/hooks/useDirectorySearch';
import AgentCapabilitiesPanel from "./AgentCapabilitiesPanel";
import WorkspaceIcon from "@/components/launcher/WorkspaceIcon";
import { useWorkspaceTreeModel } from "@/components/workspace-tree/useWorkspaceTreeModel";
import {
  WorkspaceTreeViewport,
  type WorkspaceTreeViewportHandle,
} from "@/components/workspace-tree/WorkspaceTreeViewport";
import {
  findTypeAheadTarget,
  resolveTreeKeyAction,
} from "@/components/workspace-tree/treeKeyboard";
import {
  baseNameOf,
  buildUndoPlan,
  pushUndoEntry,
  type UndoableOp,
} from "@/components/workspace-tree/undoJournal";
import {
  parentDirOfPath,
  type TreeEditingState,
} from "@/components/workspace-tree/treeTypes";
import {
  resolveExternalDropDir,
  resolveInternalDropTarget,
  ROOT_DROP_ID,
  STICKY_DROP_PREFIX,
} from "@/components/workspace-tree/dropTarget";
import {
  applyChildrenMap,
  collectFreshUpdates,
  mergeLazyChildren,
  treeNodeEqual,
} from "@/components/workspace-tree/treeMerge";
import type { VisibleTreeRow } from "@/components/workspace-tree/treeTypes";
import type {
  ContextMenuState,
  DialogState,
  ...
  FilePreview,
  TreeClipboard,
} from '@/components/directory-panel/types';

// Lazy load FilePreviewModal - it includes heavy SyntaxHighlighter/Monaco.
// Restored in the DSH port (Phase 3): FilePreviewModal
// is bundled through the `@` alias, only FilePreviewModal itself is externalized
// into a lazy chunk so the panel shell stays light.
...
const DirectoryPanel = memo(
  forwardRef<DirectoryPanelHandle, DirectoryPanelProps>(function DirectoryPanel(
    ...
    // AgentLex: when this session is docked to a case with a bound folder,
    // let the user toggle the tree root between the agent's workspace and the
    // case folder. No extra disk space — the case folder is just an alternate
    // root fed to the existing workspace_files commands (path_safety already
    // allows any existing, non-blacklisted directory as a root).
    const [viewMode, setViewMode] = useState<'workspace' | 'case'>(
      () => (caseFolder ? 'case' : 'workspace'),
    );
    // Follow the bound folder: whenever a docked owner (case/project) gains a
    // folder, surface it by default. Stable within a session, so a manual
    // toggle to 工作区 persists until the folder binding itself changes.
    useEffect(() => {
      if (caseFolder) setViewMode('case');
    }, [caseFolder]);
    const effectiveRoot = viewMode === 'case' && caseFolder ? caseFolder : agentDir;
    ...
    } = useDirectorySearch(effectiveRoot);

    // 外部工具条按钮控制搜索模式：退出时清空查询。
    useEffect(() => {
      if (searchActive === undefined) return;
    ...
    const fileService = useWorkspaceFileService(effectiveRoot ?? null);
    const workspaceChangeSignal = useWorkspaceChangeSignal(effectiveRoot ?? null, fileService.isAvailable);
    ...
    const folderName = getFolderName(effectiveRoot);
    ...
    const prevAgentDirRef = useRef(effectiveRoot);
    useEffect(() => {
      if (prevAgentDirRef.current === effectiveRoot) return;
      prevAgentDirRef.current = effectiveRoot;
      setClipboard(null);
    ...
    }, [effectiveRoot, fileService, rawRefresh]);
    ...
    // External reveal (chat path context menu → Chat → here / 会话文件链接 →
    // 侧边栏打开). Fires once per id.
    ...
    // previewNode 声明在其后，这里用 ref 间接调用，避免 TDZ。
    const previewNodeRef = useRef<((data: DirectoryTreeNode) => Promise<void>) | null>(null);
    useEffect(() => {
      ...
      // 绝对路径 → 相对 effectiveRoot（树内路径以根为基准）。
      const raw = externalRevealRequest.path.replace(/\\/g, '/');
      let rel = raw;
      if (raw.startsWith('/')) {
        const root = effectiveRoot.replace(/\\/g, '/').replace(/\/+$/, '');
        if (root !== '' && raw.startsWith(root + '/')) rel = raw.slice(root.length + 1);
      }
      ...
      void handleRevealSearchResultInTree(rel, { silentIfMissing: true });
      if (externalRevealRequest.open === true) {
        // 「用侧边栏打开」：定位后在预览弹窗中打开该文件（路径在根内才有效）。
        const name = rel.split('/').filter(Boolean).pop() ?? rel;
        void previewNodeRef.current?.({ name, path: rel, type: "file" } as DirectoryTreeNode);
      }
      onExternalRevealHandled?.(externalRevealRequest.id);
    }, [externalRevealRequest, onExternalRevealHandled, handleRevealSearchResultInTree, effectiveRoot]);
    ...
    const toAbsolutePath = useCallback(
      (relPath: string): string => {
        if (!effectiveRoot) return relPath;
        if (!relPath) return effectiveRoot;
        const sep = effectiveRoot.includes("\\") ? "\\" : "/";
        return `${effectiveRoot}${sep}${relPath}`;
      },
      [effectiveRoot],
    );
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="min-w-0 flex-1 truncate text-sm font-medium text-[var(--ink)]">
                      {viewMode === 'case' ? (caseName || folderName) : (projectDisplayName || folderName)}
                    </span>
                    {gitBranch && (
                ...
                {caseFolder && (
                  <div className="ml-auto flex flex-shrink-0 items-center rounded-lg border border-[var(--line)] p-0.5 text-xs">
                    <button
                      ...
                      className={`rounded-md px-2 py-0.5 font-medium transition-colors ${viewMode === 'workspace' ? 'bg-[var(--accent-subtle)] text-[var(--accent)]' : 'text-[var(--ink-muted)] hover:text-[var(--ink)]'}`}
                    >
                      工作区
                    </button>
                    <button
                      ...
                      className={`rounded-md px-2 py-0.5 font-medium transition-colors ${viewMode === 'case' ? 'bg-[var(--accent-subtle)] text-[var(--accent)]' : 'text-[var(--ink-muted)] hover:text-[var(--ink)]'}`}
                    >
                      {bindingKind === 'case' ? '案件文件夹' : '项目文件夹'}
                    </button>
                  </div>
                )}
          ...
          (preview || previewError || isPreviewLoading) && (
            <Suspense fallback={null}>
              <FilePreviewModal
                ...
                error={previewError}
                // Thread the absolute workspace root so rendered markdown
                // previews can load relative-path images.
                ...
                // Reveal through fileService (host /open route) rather than any
                // finder-specific sidecar.
                onRevealFile={async () => {
