...
export default function FilePreviewModal({
    ...
    // Right-click anywhere in the modal opens the same file actions as the
    // "⋯" menu — the tree's own context menu is unreachable while the modal
    // covers it, so the modal must offer one itself.
    const buildContextMenuItems = (): ContextMenuItem[] => {
        ...
        if (onQuoteFile) {
            items.push({
                ...
                onClick: () => { setContextMenu(null); void handleQuoteFileClick(); },
            });
        }
        if (onRevealInTree && !localPath) {
            items.push({
                label: t('workspaceFiles.common.revealInTree'),
                icon: <LocateFixed className="h-4 w-4" />,
                onClick: () => { setContextMenu(null); handleRevealInTree(); },
            });
        }
        items.push({
            label: t('workspaceFiles.common.copyFilePath'),
            icon: <Copy className="h-4 w-4" />,
            onClick: () => { setContextMenu(null); handleCopyFilePath(); },
        });
        if (canReveal) {
            items.push({
                label: t('workspaceFiles.common.openContainingFolder'),
                icon: <FolderOpen className="h-4 w-4" />,
                onClick: () => { setContextMenu(null); void handleOpenInFinder(); },
            });
        }
        if (canRename) {
            items.push({
                ...
                onClick: () => { setContextMenu(null); handleStartRename(); },
            });
        }
        if (!richDocKind) {
            items.push({
                ...
                icon: <Copy className="h-4 w-4" />,
                disabled: isLoading || !!error,
                onClick: () => { setContextMenu(null); void handleCopyFullText(); },
            });
        }
        return items;
    };
