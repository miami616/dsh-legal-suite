...
export async function ensureBundledWorkspace(): Promise<boolean> {
    ...
    try {
        // Lazy import to break circular dep (projectService)
        ...
        // Initialize bundled agent workspaces (诉讼管家, 非诉管家) and register each as a
        // system-preset agent at the same level as mino.
        const agentResults = await invoke<Array<{ path: string; is_new: boolean }>>('cmd_initialize_bundled_agents')
            .catch((e) => {
                console.warn('[configService] Failed to init bundled agents:', e);
                return [] as Array<{ path: string; is_new: boolean }>;
            });
        const agentPresetById = new Map<string, SystemPresetWorkspaceId>([
            ['诉讼管家', 'litigation-butler'],
            ['非诉管家', 'nonlitigation-butler'],
        ]);
        const existingProjects = await loadProjects();
        for (const ar of agentResults) {
            const folder = ar.path.split(/[\\/]/).pop() ?? '';
            const presetId = agentPresetById.get(folder);
            if (!presetId) continue;
            let proj = existingProjects.find(p => workspacePathsEqual(p.path, ar.path));
            if (!proj) {
                try {
                    proj = await addProject(ar.path);
                } catch (e) {
                    console.warn('[configService] Failed to register bundled agent:', folder, e);
                    continue;
                }
            }
            try {
                const patch = getSystemPresetProjectMetadataPatch(proj, presetId);
                if (Object.keys(patch).length > 0) {
                    await patchProject(proj.id, patch);
                }
            } catch (e) {
                console.warn('[configService] Failed to set bundled agent metadata:', folder, e);
            }
        }

        const found = existingProjects.find(p => workspacePathsEqual(p.path, result.path));
