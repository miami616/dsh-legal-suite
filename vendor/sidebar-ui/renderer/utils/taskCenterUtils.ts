...
/**
 * "N 条消息" — used by TaskCenterOverlay session rows.
 */
export function formatMessageCount(session: { stats?: { turnCount?: number } | null }): string | null {
    if (!session.stats?.turnCount || session.stats.turnCount <= 0) return null;
    return `${session.stats.turnCount} 条消息`;
}
