...
export type SystemPresetWorkspaceId = 'mino' | 'litigation-butler' | 'nonlitigation-butler';
...
export function getSystemPresetProjectMetadata(
  ...
  switch (presetId) {
    ...
    case 'litigation-butler':
      return {
        workspaceType: 'system-preset',
        systemPresetId: 'litigation-butler',
        icon: 'detective',
        displayName: '诉讼管家',
        templateId: 'litigation-butler',
        templateSource: 'builtin',
      };
    case 'nonlitigation-butler':
      return {
        workspaceType: 'system-preset',
        systemPresetId: 'nonlitigation-butler',
        icon: 'folder-open',
        displayName: '非诉管家',
        templateId: 'nonlitigation-butler',
        templateSource: 'builtin',
      };
  }
}
...
export function getSystemPresetProjectMetadataPatch(
  ...
  const patch: ProjectPatch = {};
    ...
    // 普通项目保留"只在空时填默认"的语义；system-preset 项目所有字段由代码权威控制，
    // 必须强制同步——否则磁盘残留旧图标名（如曾用过的 scale/scroll）会永久卡死、
    // 且 c0889d66/bdca0cb4 等图标升级也永远不会反映到运行时注册表 projects.json。
    if (project.workspaceType === 'system-preset') {
        if (metadata.icon && project.icon !== metadata.icon) patch.icon = metadata.icon;
        if (metadata.displayName && project.displayName !== metadata.displayName) patch.displayName = metadata.displayName;
    } else {
...
export const PRESET_TEMPLATES: WorkspaceTemplate[] = [
  ...
  {
    id: 'litigation-butler',
    name: '诉讼管家',
    description: '专注诉讼业务的 AI 管家。整理案卷、梳理争议焦点、起草诉讼文书。',
    icon: 'detective',
    isBuiltin: true,
    agentDefaults: {
      enabled: true,
  ...
  {
    id: 'nonlitigation-butler',
    name: '非诉管家',
    description: '专注非诉业务的 AI 管家。审查合同、起草法律意见、管理交易流程。',
    icon: 'folder-open',
    isBuiltin: true,
    agentDefaults: {
      enabled: true,
...
export interface AppConfig {
  // AgentLex: legacy business-module → agent-dir bindings (see renderer/utils/businessModules.ts)
  businessAgentBindings?: Record<string, string>;
  ...
  // UI preferences
  /** Custom brand name shown in the Launcher greeting (e.g. "下午好，{brandName}"). */
  brandName?: string;
