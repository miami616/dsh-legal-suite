...
/**
 * 保证危险的保留名黑名单。`~/.myagents/agentlex-v2/bin` 和 `~/.myagents/bin` 在 PATH 中先于系统路径，
...
export const CLI_TOOL_RESERVED_NAMES: ReadonlySet<string> = new Set([
  'agentlex-v2', 'myagents',
  // 外部 Runtime 的 CLI：重名工具会被 resolveCommand 解析到（agentlex-v2/bin
  // 和 .myagents/bin 在 fallback PATH 前列），导致 spawn 的是注册工具而不是真 runtime——必然灾难级。
  'claude', 'codex', 'gemini',
